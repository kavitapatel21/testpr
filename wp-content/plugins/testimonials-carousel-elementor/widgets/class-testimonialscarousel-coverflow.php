      ...
      <section class="swiper mySwiper mySwiperCoverflow <?php if (
        ...
        <ul class="swiper-wrapper">
          ...
          foreach ($settings['slide'] as $item) {
            $custom_attributes = Utils::parse_custom_attributes(wp_kses($item['slide_button_link']['custom_attributes'], []));
            ?>
            <li class="swiper-slide slider-container-background">
              <div class="block-shadow slider-container-block-background"
                   <?php if (esc_url($item['slide_image']['url'])) { ?>style="background: url('<?php echo esc_url($item['slide_image']['url']); ?>') no-repeat 50%50%/cover;" <?php } ?>>
                <div class="slider-coverflow-wrapper">
                  <div>
                    <h2 class="slide-title"><?php echo wp_kses($item['slide_name'], []); ?></h2>
                    <div class="slide-description"
                         style="line-height: 22px;"><?php echo $item['slide_content']; ?></div>
                  </div>
                  <?php if (wp_kses($item['slide_show_button'], []) === 'yes') { ?>
                    <div class="slide-coverflow-button-wrapper">
                      <a id="<?php echo wp_kses($item['slide_button_css_id'], []); ?>"
                         class="elementor-button slide-coverflow-button <?php if (!empty($settings['slide_button_hover_animation'])) { ?> elementor-animation-<?php echo $settings['slide_button_hover_animation'];
                         } ?>"
                        <?php if (wp_kses($item['slide_button_link']['is_external'], []) === 'on') { ?> target="_blank"
                         <?php } if (wp_kses($item['slide_button_link']['nofollow'], [])) { ?>rel="nofollow"<?php } ?>
                         href="<?php if (!empty(wp_kses($item['slide_button_link']['url'], []))) {
                           echo wp_kses($item['slide_button_link']['url'], []);
                         } else {
                           echo '#';
                         } ?>"
                         role="button"
