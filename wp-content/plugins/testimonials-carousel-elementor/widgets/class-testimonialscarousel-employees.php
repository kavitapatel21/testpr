...
class TestimonialsCarousel_Employees extends Widget_Base
{
  ...
  public function __construct($data = [], $args = null)
  {
    ...
    wp_register_style('testimonials-carousel-employees', plugins_url('/assets/css/testimonials-carousel-employees.min.css', TESTIMONIALS_CAROUSEL_ELEMENTOR), [], '7.1.0');
                    ...
                    <div class="media-icons">
                      ...
                      if ($item['social_to_show'] > '0') {
                        for ($i = 1; $i <= (int)$item['social_to_show']; $i++) {
                          $custom_attributes_social = Utils::parse_custom_attributes(wp_kses($item['social_link_' . $i]['custom_attributes'], []));
                          ?>
                          <a <?php if (wp_kses($item['social_link_' . $i]['is_external'], []) === 'on') { ?> target="_blank"
                            <?php } if (wp_kses($item['social_link_' . $i]['nofollow'], [])) { ?>rel="nofollow"<?php } ?>
                            href="<?php if (!empty(wp_kses($item['social_link_' . $i]['url'], []))) {
                              echo wp_kses($item['social_link_' . $i]['url'], []);
                            } else {
                              echo '#';
                            } ?>"
                            role="button"
                            <?php foreach ($custom_attributes_social as $key_attr => $val_attr) {
                              echo $key_attr . "='$val_attr'";
                            } ?>>
                            <span>
                              <i class="<?php echo $item['social_icon_' . $i]['value']; ?>"></i>
                            </span>
                          </a>
                          <?php
                        }
                  ...
                  <div class="button">
                    ...
                    if ($item['slide_button_to_show'] > '0') {
                      for ($i = 1; $i <= (int)$item['slide_button_to_show']; $i++) {
                        $custom_attributes = Utils::parse_custom_attributes(wp_kses($item['slide_button_link_' . $i]['custom_attributes'], []));
                        ?>
                        <a <?php if (wp_kses($item['slide_button_link_' . $i]['is_external'], []) === 'on') { ?> target="_blank"
                          <?php } if (wp_kses($item['slide_button_link_' . $i]['nofollow'], [])) { ?>rel="nofollow"<?php } ?>
                          href="<?php if (!empty(wp_kses($item['slide_button_link_' . $i]['url'], []))) {
                            echo wp_kses($item['slide_button_link_' . $i]['url'], []);
                          } else {
                            echo '#';
                          } ?>"
                          role="button"
                          <?php foreach ($custom_attributes as $key_attr => $val_attr) {
                            echo $key_attr . "='$val_attr'";
                          } ?>
                        >
