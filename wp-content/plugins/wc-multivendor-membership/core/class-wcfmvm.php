...
class WCFMvm {
	...
	public function load_plugin_textdomain() {
		...
		load_textdomain( 'wc-multivendor-membership', WP_LANG_DIR . "/plugins/wc-multivendor-membership-$locale.mo");
	}
  ...
  public function wcfmvm_ipn_listener() {
		...
		if( $wcfm_membership ) {
			...
			if( !$has_error ) {
				...
				if( ( $wcfm_membership != -1 ) && ( $wcfm_membership != '-1' ) ) {
					...
					$message = str_replace( '{dashboard_url}', '<a href="' . esc_url(get_wcfm_url()) . '">' . __( 'Visit now ...', 'wc-multivendor-membership' ) . '</a>', $subscription_welcome_email_content );
					...
					// Switch language context…
					if( apply_filters( 'wcfm_allow_wpml_email_translation', true ) ) {
						do_action('wpml_switch_language_for_email', apply_filters( 'wcfm_admin_email_notification_receiver', get_bloginfo( 'admin_email' ), 'membership' ));
					}
					...
					$message = str_replace( '{store_url}', wcfmmp_get_store_url( $member_id ), $message );
					$message = str_replace( '{membership_plan}', get_the_title( $wcfm_membership ), $message );
					$message = str_replace( '{vendor_url}', '<a href="' . esc_url(get_wcfm_vendors_manage_url($member_id)) . '">' . __( 'Visit now ...', 'wc-multivendor-membership' ) . '</a>', $message );
					if( !$wcfm_register_member ) {
					...
					if( !$wcfm_register_member ) {
						$wcfm_messages = sprintf( __( '<b>%s</b> (Store: <b>%s</b>) subscribed for <b>%s</b> membership plan.', 'wc-multivendor-membership' ), '<a href="' . esc_url(get_wcfm_vendors_manage_url($member_id)) . '" target="_blank">' . $member_user->first_name . '</a>', $shop_name, get_the_title( $wcfm_membership ) );
					} elseif( $is_membership_renewal ) {
						$wcfm_messages = sprintf( __( '<b>%s</b> (Store: <b>%s</b>) has renewed membership plan to <b>%s</b>.', 'wc-multivendor-membership' ), '<a href="' . esc_url(get_wcfm_vendors_manage_url($member_id)) . '" target="_blank">' . $member_user->first_name . '</a>', $shop_name, get_the_title( $wcfm_membership ) );
					} else {
						$wcfm_messages = sprintf( __( '<b>%s</b> (Store: <b>%s</b>) has changed membership plan to <b>%s</b>.', 'wc-multivendor-membership' ), '<a href="' . esc_url(get_wcfm_vendors_manage_url($member_id)) . '" target="_blank">' . $member_user->first_name . '</a>', $shop_name, get_the_title( $wcfm_membership ) );
					}
					...
					$subject = apply_filters( 'wcfm_email_subject_wrapper', $subject );
					$message = str_replace( '{dashboard_url}', '<a href="' . esc_url(get_wcfm_url()) . '">' . __( 'Visit now ...', 'wc-multivendor-membership' ) . '</a>', $registration_email_content );
					$message = str_replace( '{first_name}', $member_user->first_name, $message );
					$message = str_replace( '{site_name}', get_bloginfo( 'name' ), $message );
					$message = str_replace( '{store}', wcfm_get_vendor_store( $member_id ), $message );
					$message = str_replace( '{store_name}', wcfm_get_vendor_store_name( $member_id ), $message );
					...
					// Switch language context…
					if( apply_filters( 'wcfm_allow_wpml_email_translation', true ) ) {
						do_action('wpml_switch_language_for_email', apply_filters( 'wcfm_admin_email_notification_receiver', get_bloginfo( 'admin_email' ), 'registration' ));
					}
					...
					// Admin Desktop Notification
					$wcfm_messages = sprintf( __( '<b>%s</b> (Store: <b>%s</b>) successfully registered to our site.', 'wc-multivendor-membership' ), '<a class="wcfm_dashboard_item_title" href="' . esc_url(get_wcfm_vendors_manage_url($member_id)) . '" target="_blank">' . $member_user->first_name . '</a>', $shop_name );
	...
	function wcfmvm_registration_process_on_order_completed( $order_id ) {
		...
		$wcfm_membership_order_processed = $order->get_meta( '_wcfm_membership_order_processed' );
		...
		$order->update_meta_data( '_wcfm_membership_order_processed', 'yes' );
		$order->save();
	}
	...
	function send_approval_reminder_admin( $member_id ) {
		...
		if( apply_filters( 'wcfm_is_allow_notification_email', true, 'vendor_approval' ) ) {
			...
			// Switch language context…
			if( apply_filters( 'wcfm_allow_wpml_email_translation', true ) ) {
				do_action('wpml_switch_language_for_email', apply_filters( 'wcfm_admin_email_notification_receiver', get_bloginfo( 'admin_email' ), 'vendor_approval' ));
			}
			...
			$message = str_replace( '{notification_url}', '<a href="' . esc_url(get_wcfm_messages_url()) . '">' . __( 'Check Here ...', 'wc-multivendor-membership' ) . '</a>', $message );
				...
				if( $free_expiry_reminder || ( ( $is_free == 'no' ) && ( $subscription_type != 'one_time' ) ) ) {
					...
					if( $next_schedule ) {
						...
						if( $send_reminder ) {
							...
							// Switch language context…
							if( apply_filters( 'wcfm_allow_wpml_email_translation', true ) ) {
								do_action('wpml_switch_language_for_email', $member->user_email);
							}
