jQuery(document).ready(function ($) {
	...
	// Request Withdrawals Approve Manually
	$('#wcfm_withdrawal_requests_manually_approve_button').click(function (event) {
		event.preventDefault();

		if( ! confirm( 'Are you sure you want to approve withdrawals manually? This action can not be reverted.' ) ) return;

		$('#wcfm_withdrawal_requests_approve_button').hide();
		$('#wcfm_withdrawal_requests_cancel_button').hide();
		$('#wcfm_withdrawal_requests_manually_approve_button').hide();
		$('.bulk_action_checkbox_all').attr('checked', false);
		...
		var data = {
			...
			controller: 'wcfm-withdrawal-requests-manually-approve',
		...
		$.post(wcfm_params.ajax_url, data, function (response) {
			if (response) {
				...
				if (response.success) {
					$('#wcfm_withdrawal_requests_manage_form .wcfm-message').html('<span class="wcicon-status-completed"></span>' + response.data.message).addClass('wcfm-success').slideDown();
					$wcfm_withdrawal_requests_table.ajax.reload();
				} else {
					$('#wcfm_withdrawal_requests_manage_form .wcfm-message').html('<span class="wcicon-status-cancelled"></span>' + response.data.message).addClass('wcfm-error').slideDown();
				}
				wcfmMessageHide();
				$('#withdraw_note').val('');
				...
				$('#wcfm_withdrawal_requests_manually_approve_button').show();
			}
		});
	});
