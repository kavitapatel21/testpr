...
class WCFMu_Vendor_Invoice {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_wcfm_order_pdf_three', array( &$this, 'wcfm_order_pdf_three' ) );
		add_action( 'wp_ajax_wcfm_order_pdf_four', array( &$this, 'wcfm_order_pdf_four' ) );
		add_action( 'wp_ajax_wcfm_order_pdf_two', array( &$this, 'wcfm_order_pdf_two' ) );
		add_action( 'wp_ajax_wcfm_order_pdf_five', array( &$this, 'wcfm_order_pdf_five' ) );
		//add_action( 'wp_ajax_wcfm_order_pdf_six', array( &$this, 'wcfm_order_pdf_six' ) );
	...
	function wcfmu_pdf_invoice_orders_actions( $actions, $order_id, $the_order, $vendor_id = 0 ) {
		...
		if( apply_filters( 'wcfm_is_allow_pdf_invoice', true ) && apply_filters( 'wcfm_is_allow_view_commission', true ) ) {
			if( wcfm_is_vendor() ) {
				...
				//$pdf_actions .= '<a class="wcfm_pdf_invoice_six wcfm-action-icon '. $item_class . '" href="#" data-vendorid="' . $vendor_id . '" data-orderid="' . $order_id . '"><span class="wcfmfa fa-file-powerpoint text_tip" data-tip="出荷指示書"></span></a>';
						
			} else {				
				
				$slug = $_SERVER['REQUEST_URI'];
				$order_details = explode("/",$slug);
				$order = $order_details[2];
				
				if($order == 'orders-details'){
					
				$pdf_actions .= '<a class="wcfm_pdf_invoice wcfm-action-icon'. $item_class . '" href="#" data-vendorid="' . $vendor_id . '" data-orderid="' . $order_id . '"><span class="wcfmfa fa-file-powerpoint text_tip" data-tip="領収書"></span></a>';	
				
				$pdf_actions .= '<a class="wcfm_pdf_invoice_four wcfm-action-icon'. $item_class . '" href="#" data-vendorid="' . $vendor_id . '" data-orderid="' . $order_id . '"><span class="wcfmfa fa-file-powerpoint text_tip" data-tip="納品書"></span></a>';
				
				$pdf_actions .= '<a class="wcfm_pdf_invoice_five wcfm-action-icon'. $item_class . '" href="#" data-vendorid="' . $vendor_id . '" data-orderid="' . $order_id . '"><span class="wcfmfa fa-file-powerpoint text_tip" data-tip="出荷指示書"></span></a>';
					
				
				$pdf_actions .= '<a class="wcfm_pdf_invoice_three wcfm-action-icon'. $item_class . '" href="#" data-vendorid="' . $vendor_id . '" data-orderid="' . $order_id . '"><span class="wcfmfa fa-file-powerpoint text_tip" data-tip="請求書"></span></a>';
					
				$pdf_actions .= '<a class="wcfm_pdf_invoice_two wcfm-action-icon'. $item_class . '" href="#" data-vendorid="' . $vendor_id . '" data-orderid="' . $order_id . '"><span class="wcfmfa fa-file-powerpoint text_tip" data-tip="見積書"></span></a>';
				...
				//$pdf_actions .= '<a class="popupVendor wcfm-action-icon" href="#!" orderid="' . $order_id . '"><span class="wcfmfa fa-file-powerpoint text_tip" data-tip=""></span></a>';
				...
				if( is_a( $order , 'WC_Order' ) ) {
					...
					if( !empty( $items ) ) {
						foreach( $items as $order_item_id => $item ) {
							...
							$v_name = ($processed_vendors[$vendor_id]);
							$vendor = get_userdata($v_name);
							$v_display_name = $vendor->display_name;
							
							$vendorHtml .= '<ul><li><a class="wcfm_pdf_invoice_six wcfm-action-icon '. $item_class . '" href="#" data-vendorid="' . $vendor_id . '" data-orderid="' . $order_id . '"><span class="wcfmfa fa-file-powerpoint text_tip" data-tip="出荷指示書"></span>'.$v_display_name.'</a></li></ul>';
				...
				//$pdf_actions .= '<div class="messagepop pop" id="pop_'.$order_id.'">'.		$vendorHtml							.'</div>';
			}
		}
	...
	function wcfm_order_pdf_invoice() {
		global $WCFM, $WCFMu, $wpo_wcpdf, $order, $order_id, $vendor_id, $document, $document_type, $plugin_path, $plugin_url, $wpdb;
		...
		$filename = __( '領収書', 'wc-frontend-manager-ultimate' ).'_'.date('YmdHis') . '_' . $order_id . '.pdf';
	...
	/**
	 * WooCommerce PDF Invoices & 2
	 * Download PDF Invoice 2
	 */
	function wcfm_order_pdf_two() {
		
		global $WCFM, $WCFMu, $wpo_wcpdf, $order, $order_id, $document, $document_type, $wpdb;
		...
		// Fetching Main template
		if( is_rtl() ) {
			$template = $WCFMu->template->locate_template( 'vendor_invoice/invoice-two-rtl.php' );
		} else {
			$template = $WCFMu->template->locate_template( 'vendor_invoice/invoice-two.php' );
		}
		ob_start();
		if (file_exists($template)) {
			include($template);
		...
		//$filename = __( 'invoice-two', 'wc-frontend-manager-ultimate' ) . '-' . $order_id . '.pdf';
		$filename = __( '見積書', 'wc-frontend-manager-ultimate' ).'_'.date('YmdHis') . '_' . $order_id . '.pdf';
	...
	/**
	 * WooCommerce PDF Invoices & 3
	 * Download PDF Invoice 3
	 */
	function wcfm_order_pdf_three() {
		global $WCFM, $WCFMu, $wpo_wcpdf, $order, $order_id, $document, $document_type, $wpdb;
  	...
  	$order_id = $_GET['order_id'];
		...
		// Fetching Main template
		if( is_rtl() ) {
			$template = $WCFMu->template->locate_template( 'vendor_invoice/invoice-three-rtl.php' );
		} else {
			$template = $WCFMu->template->locate_template( 'vendor_invoice/invoice-three.php' );
		}
		ob_start();
		if (file_exists($template)) {
			include($template);
		...
		//$filename = __( 'invoice-three', 'wc-frontend-manager-ultimate' ) . '-' . $order_id . '.pdf';
		$filename = __( '請求書', 'wc-frontend-manager-ultimate' ).'_'.date('YmdHis') . '_' . $order_id . '.pdf';
	...
	/**
	 * WooCommerce PDF Invoices & 4
	 * Download PDF Invoice 4
	 */
	function wcfm_order_pdf_four() {
		global $WCFM, $WCFMu, $wpo_wcpdf, $order, $order_id, $document, $document_type, $wpdb;
  	...
  	$order_id = $_GET['order_id'];
		...
		// Fetching Main template
		if( is_rtl() ) {
			$template = $WCFMu->template->locate_template( 'vendor_invoice/invoice-four-rtl.php' );
		} else {
			$template = $WCFMu->template->locate_template( 'vendor_invoice/invoice-four.php' );
		}
		ob_start();
		if (file_exists($template)) {
			include($template);
		...
		//$filename = __( 'invoice-four', 'wc-frontend-manager-ultimate' ) . '-' . $order_id . '.pdf';
		$filename = __( '納品書', 'wc-frontend-manager-ultimate' ).'_'.date('YmdHis') . '_' . $order_id . '.pdf';
	...
	/**
	 * WooCommerce PDF Invoices & 3 and 5
	 * Download PDF Invoice 3 and 5
	 */
	function wcfm_order_pdf_five() {
		...
		// Fetching Main template
		if( is_rtl() ) {
			$template = $WCFMu->template->locate_template( 'vendor_invoice/invoice-five-rtl.php' );
		} else {
			$template = $WCFMu->template->locate_template( 'vendor_invoice/invoice-five.php' );
		}
		ob_start();
		if (file_exists($template)) {
			include($template);
		...
		//$filename = __( 'invoice-five', 'wc-frontend-manager-ultimate' ) . '-' . $order_id . '.pdf';
		$filename = __( '出荷指示書', 'wc-frontend-manager-ultimate' ).'_'.date('YmdHis') . '_' . $order_id . '.pdf';
	...
	/*function wcfm_order_pdf_six() {
  	...
  	$order_id = $_GET['order_id'];
		...
		if( is_rtl() ) {
			$template = $WCFMu->template->locate_template( 'vendor_invoice/invoice-six-rtl.php' );
		} else {
			$template = $WCFMu->template->locate_template( 'vendor_invoice/invoice-six.php' );
		}
		ob_start();
		if (file_exists($template)) {
			include($template);
