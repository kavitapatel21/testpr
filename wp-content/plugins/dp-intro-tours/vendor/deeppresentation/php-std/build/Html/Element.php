<?php namespace IntroToursDP\Std\Html;

use IntroToursDP\Std\Html\Attr;
...
class Element {
	...
	//@param array|IntroToursDP\Std\Html\Attr $attributes
	public function __construct( string $element, string $BEMBase = null, $attributes = null, $content = null, $BEMMod = array(), $hasClosing = true ) {
		...
		if ( $attributes ) {
			...
			} elseif ( is_a( $attributes, 'IntroToursDP\Std\Html\Attr' ) ) {
	...
	private function _add_one_content( $content ) {
		if ( is_string( $content ) || is_a( $content, 'IntroToursDP\Std\Html\Element' ) ) {
			if ( is_a( $content, 'IntroToursDP\Std\Html\Element' ) ) {
				$content->parent = $this;
			}
