...
class Color {
	...
	public function toArray( $format = null ) {
		...
		if ( $format === 'hex' ) {
			$format = 'rgba'; }
		$color = $this->color;
		if ( in_array( $format, [ 'hsl', 'hsla' ], true ) ) {
			$color = static::rgba2hsla( $color );
		} elseif ( in_array( $format, [ 'hsv', 'hsva' ], true ) ) {
			$color = static::rgba2hsva( $color );
		}
		if ( ! in_array( $format, static::$alpha_formats, true ) ) {
			unset( $color[3] );
		}
		return $color;
	}
	...
	public static function standarizeFormat( $format ) {
		...
		if ( ! in_array( $format, array_keys( static::$color_formats ) ) ) {
			$format = 'rgba';
		}
		return $format;
	}
	public static function detectFormat( $color ) {
		foreach ( static::$color_formats as $format => $detector ) {
	...
	public static function rgba2text( $color, $format = 'rgba' ) {
		$format = static::standarizeFormat( $format );
		if ( ! in_array( $format, static::$alpha_formats ) ) {
			unset( $color[3] ); } else {
			$color[3] /= 100; }
			switch ( $format ) {
				case 'rgba':
				...
				case 'rgb':
					return vsprintf( 'rgb(%s, %s, %s)', $color );
				case 'hex':
					if ( isset( $color[3] ) ) {
						unset( $color[3] );
					}
	...
	public static function rgba2hsla( $rgba ) {
		...
		$l   *= 100;
		$hsla = [ $h, $s, $l ];
		foreach ( $hsla as $i => $segment ) {
			$hsla[ $i ] = round( $segment ); }
		$hsla[3] = 100;
		if ( isset( $rgba[3] ) ) {
			$hsla[3] = $rgba[3]; }
		return $hsla;
