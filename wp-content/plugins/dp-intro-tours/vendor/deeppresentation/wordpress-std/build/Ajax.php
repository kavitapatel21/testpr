...
use IntroToursDP\Std\Core\Autoloader;
use IntroToursDP\Std\Process\AjaxResp;
...
class Ajax {
	...
	*/
	...
	public function __construct() {
		 add_action( 'wp_ajax_run_task', [ $this, 'run_task' ] );
		add_action( 'wp_ajax_nopriv_run_task', [ $this, 'run_task_nopriv' ] );
	}
	...
	public function register_task( string $taskAndScriptName, string $componentClassName, array $args ) {
		if ( ! empty( $taskAndScriptName ) && ! empty( $componentClassName ) && $args ) {
			Autoloader::Instance()->require_once( [ $componentClassName ] );
			$this->ajaxTasks[ $taskAndScriptName ] = [
				'component' => [
	...
	public function enable_tasks( array $taskIdsList, string $hookAction ) {
		...
		add_action( $hookAction, [ $this, 'admin_equeue_scripts' ] );
	}
	...
	public function admin_equeue_scripts() {
		foreach ( $this->ajaxTasks as $taskId => $taskCfg ) {
			if ( $taskCfg['enabled'] ) {
				...
				$scriptDependency = Arr::sget( $taskCfg, 'args.script-dependency', [] );
				...
				if ( ! empty( $actionName ) ) {
					...
					if ( $componentInstance ) {
						...
						if ( method_exists( $componentInstance, $actionName ) ) {
							$response = call_user_func( [ $componentInstance, $actionName ], Arr::as_array( $actionArgs ) );
