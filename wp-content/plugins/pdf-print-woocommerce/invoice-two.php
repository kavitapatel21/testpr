			...
			<td class="address billing-address" width="30%" style="margin-top:20px;">
				<table class="totals" style="margin-top:20px;width:100%;">
					<tfoot>
						...
						<?php $cus_form_info_bill_date  = get_post_meta( $order_id[0]->order_id, '_cus_form_info_bill_date'); 
							if($cus_form_info_bill_date [0]){
							?>
							<tr>
								<th class="description textcenter">請求日</th>
								<td class="price" style="border:1px solid #ccc;"><span class="invoice-date"><?php echo date_i18n(wc_date_format(), strtotime($cus_form_info_bill_date[0])); ?></span></td>
							</tr>
							<?php }else{ ?>
		...
		<tr>
			<td class="address billing-address">
				<table class="totals remove-border" style="width:100%" border="0">
					<tfoot>
						<tr>
							<th style="width:7.4%" class="description textcenter">件名</th>
						...
						<tr class="payment-method">
							<th style="width:7.4%" class="description textcenter">決済方法</th>
							<td class="price"><?php echo $payment_title = $order->get_payment_method_title(); ?></td>
						</tr>
						<?php 
						$payment_title = $order->get_payment_method_title();
						$word = "銀行振込";
						if(strpos($payment_title, $word) !== false){ 
						?>						
						<tr>
							<th style="width:10%" class="description textcenter">支払期限</th>
							<td class="price">
							<?php
							$cus_form_last_date = get_post_meta( $pdf, '_cus_form_last_date' ); 
							//echo "<pre>";
							//print_r($cus_form_last_date);
							if($cus_form_last_date[0]){
								 echo date_i18n(wc_date_format(), strtotime($cus_form_last_date[0])); 
							}else{
							$day = get_option( 'pdf-print-woocommerce-option-day' );
							if($day ){
								 $post = get_post($order_id[0]->order_id);
								echo date_i18n(wc_date_format(), strtotime("+". $day ."day", strtotime($post->post_date)));
							}else{
							$date = strtotime($post->post_date);
							echo date_i18n(wc_date_format(), strtotime("+7 day", $date)); 
							}
							}
							?>
							</td>
						...
						<tr class="payment-method">
							<th style="width:10%" class="description textcenter">振込先</th>	
							<td class='price'>
							<?php 				
						$account = get_option( 'woocommerce_bankjp_accounts' );
						if (!empty($account)) {
							$counter = 1;
							foreach ($account as $ac => $val){
								
								echo $val['bank_name'] ." ".$val['bank_branch']." ".$val['bank_type']." ".$val['account_number']." ".$val['account_name']; 
								echo "<br>";
								if($counter++ == 5) {
								break;
								}
...
<table style="width:100%;margin-top: 25px;">
	<thead>
		...
		<tr>
			...
			<?php 
			$cus_form_info_invoice_detail = get_post_meta( $order_id[0]->order_id, '_cus_form_info_invoice_detail');
			echo $cus_form_info_invoice_detail[0];
			?>
			</td>
		</tr>	
		<tr>
		
			<?php $payment_title = $order->get_payment_method_title(); ?>
			<?php if($payment_title == "銀行振込（日本国内）") { ?>
			<td class="product" style="text-align:left; background-color: #fff; color:black; border:none;">＊お振込手数料はご負担願います</td>
			<?php } ?>
		</tr>			
	</thead>
</table>
